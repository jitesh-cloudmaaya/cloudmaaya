...
class EProductSearch(FacetedSearch):
    ...
    def query(self, search, query):
        ...
        if self._card_count:
            return search.query(main_q).query(q_faves).query(q_available).query(q_not_deleted).query(q_not_final_sale).extra(collapse=collapse_dict).extra(aggs=cardinality_dict)
        else:
            #search.aggs.bucket("unique_product_name_count", {"cardinality" : {"field" : "product_name.keyword"}})
            return search.query(main_q).query(q_faves).query(q_available).query(q_not_deleted).query(q_not_final_sale).query(custom_score_dict).extra(collapse=collapse_dict).sort(self._sort)
        #.sort('-p')
