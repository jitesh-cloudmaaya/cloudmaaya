...
- catalogue_service/tasks/
- catalogue_service/tasks/product_feed_py/
- catalogue_service/tasks/product_feed_py/merchants_config/
- catalogue_service/tasks/product_feed_sql/

Most of the relevant ProductFeed code lives in the catalogue_service/tasks directory. The catalogue_service/tasks/tasks.py file is where all the product data ingest tasks live. To understand the product feed jobs, it is probably best to start there. The catalogue_service/tasks/product_feed.py file defines the ProductFeed class that is initialized at the start of every task.

Configuration for a given network (RAN, Pepperjam, etc.) are located in a separate directory, catalogue_service/catalogue_service/. They are identified by the filename <network_name>.yaml. Their filepath is passed as an argument to a ProductFeed object to define the differences between the networks.

Additional relevant files are located under catalogue_service/tasks/product_feed_py/. It includes the different specific methods used to clean the data, identified by the filename <network_name>.py, and the product_feed_helpers.py and mappings.py files contain helper methods used throughout the specific network cleaning method.

The catalogue_service/tasks/product_feed_py/merchants_config contains merchant specific configuration files on how to process a given merchant's raw data, with the specifics of those files structures dependent on the merchant's network.

The catalogue_service/tasks/product_feed_sql/ directory contains SQL scripts that are used in support of loading the product data from the flat file generated by the ProductFeed processes to the Product database table. The specific files of interest are the load-cleaned-data-<n>.sql series.
