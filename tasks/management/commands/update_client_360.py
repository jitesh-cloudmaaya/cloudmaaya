...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            #                       {'user_filter': 'WHERE wu.id IN (SELECT u0.ID FROM wp_users u0 WHERE u0.last_modified > (SELECT MAX(a0.last_updated) FROM allume_client_360 a0));'
            #                           , 'quiz_answer_user_filter_and_clause': 'and qua.user_email IN (SELECT u0.user_email FROM wp_users u0 WHERE u0.last_modified > (SELECT MAX(a0.last_updated) FROM allume_client_360 a0));',
            #                        'quiz_answer2_user_filter_and_clause': 'and qua.user_emai IN (SELECT u0.user_email FROM wp_users u0 WHERE u0.last_modified > (SELECT MAX(a0.last_updated) FROM allume_client_360 a0));'})
