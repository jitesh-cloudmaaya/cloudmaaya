...
INSERT INTO update_subset (wp_user_id)
  SELECT ID
  FROM wp_users
  WHERE last_modified > (SELECT MAX(last_updated)
                         FROM allume_client_360);

DELETE FROM allume_client_360
WHERE wp_user_id IN (SELECT wp_user_id
                     FROM update_subset);
...
INSERT INTO allume_client_360 (
  ...
  FROM
    ...
    GROUP BY ol.wp_user_id, ol.order_date) last_ord_amt
  ...
  WHERE wu.id IN (SELECT wp_user_id
                  FROM update_subset);
