...
# take 3 version of script after meeting to discuss desired logic
from product_api.models import Product, CategoryMap, AllumeCategory, SynonymCategoryMap
from django.db.models import Q
...
for category_pair in category_pairs:
    ...
    if not categorymap.allume_category or categorymap.allume_category == 'Unsure':
        # then we are fine to go ahead and re-define this categorymap entry
        pass
    else:
        # we want to try and make a new categorymap separate from this already mapped categorymap


    # but we should try to resolve duplicates in any case, b/c primary category, 'Other' may already exist...
    # b/c Pamela wanted to remap those anyway, 
